...
public class UTXOSet {
    ...
    public void update(Block tipBlock) {
        if (tipBlock == null) {
           // log.error("Fail to update UTXO set ! tipBlock is null !");
            System.out.println("Fail to update UTXO set ! tipBlock is null !");
    ...
    @Synchronized
    public void updateByTransaction (Transaction newTransaction) {

        if (newTransaction == null) {
            // log.error("Fail to update UTXO set ! tipBlock is null !");
            System.out.println("Fail to update UTXO set ! newTransaction is null !");
            throw new RuntimeException("Fail to update UTXO set ! ");
        }
        ...
        if (!newTransaction.isCoinbase()) {
            for (TXInput txInput : newTransaction.getInputs()) {
                // 余下未被使用的交易输出
                TXOutput[] remainderUTXOs = {};
                String txId = Hex.encodeHexString(txInput.getTxId());
                TXOutput[] txOutputs = RocksDBUtils.getInstance().getUTXOs(txId);
        ...
        // 新的交易输出保存到DB中
        TXOutput[] txOutputs = newTransaction.getOutputs();
        String txId = Hex.encodeHexString(newTransaction.getTxId());
        RocksDBUtils.getInstance().putUTXOs(txId, txOutputs);
    }
