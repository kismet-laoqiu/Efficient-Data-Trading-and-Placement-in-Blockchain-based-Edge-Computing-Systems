...
import com.example.demo.blockchain.transaction.TXInput;
...
public class Blockchain {
    ...
    public static Blockchain initBlockchainFromDB() {
        ...
        if (lastBlockHash == null) {
            System.out.println("ERROR: Fail to init blockchain from db. ");
    ...
    public static Blockchain createBlockchain(String address) {
        ...
        if (StringUtils.isBlank(lastBlockHash)) {
            // 创建 coinBase 交易
            System.out.println("创建coinbase交易");
            ...
            Block genesisBlock = Block.newGenesisBlock(address,coinbaseTX);
    ...
    public static Blockchain getBlockchain(String address) {
        String lastBlockHash = RocksDBUtils.getInstance().getLastBlockHash();
        if(StringUtils.isBlank(lastBlockHash)) {
            throw new RuntimeException("区块链没有创建");
        }
        //todo
        //return RocksDBUtils.getInstance();
        return new Blockchain(lastBlockHash);
...
//            if(RocksDBUtils.getInstance().getTransaction(Arrays.toString(tx.getTxId()))!=null) {
//                RocksDBUtils.getInstance().deleteTransactions(Arrays.toString(tx.getTxId()));
//            }
        }
        String lastBlockHash = RocksDBUtils.getInstance().getLastBlockHash();
        if (lastBlockHash == null) {
            System.out.println("ERROR: Fail to get last block hash ! ");
            throw new RuntimeException("ERROR: Fail to get last block hash ! ");
        }

        Block block = Block.newBlock(lastBlockHash, transactions, RocksDBforContent.getInstance().getCauseContent());
        this.addBlock(block);
        return block;
    }
    ...
    private Transaction findTransaction(byte[] txId) {
        ...
        for (Transaction tx : RocksDBUtils.getInstance().getUndeletedTransactions()) {
            if (Arrays.equals(tx.getTxId(), txId)) {
                return tx;
            }
        }
    ...
    private boolean verifyTransactions(Transaction tx) {
        ...
        try {
            ...
            System.out.println("Fail to verify transaction ! transaction invalid ! "+e);
